package com.hjy.android.expandrecycler;
...
public class MyRecyclerView extends RecyclerView {
    ...
    private final static float dragRatio = 0.5f;
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!canScrollVertically(-1)) {
                    ...
                    if (Math.abs(dy) > Math.abs(dx)) {
                        isRefresh = true;
                        if (dy<0) {// 向下滑动
                            changeHeight((int) (dy*dragRatio));
                            maxHeaderViewHeight = headerView.getHeight();
                        }else {
                        }
                    }
                }
                if (isRefresh && maxHeaderViewHeight >headerViewHeight){
                    // lastY-Y>0向上滑动；lastY-Y<0向下滑动
                    maxHeaderViewHeight = maxHeaderViewHeight - dy;
                    setStateByHeight(maxHeaderViewHeight, false);
                }
                break;
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                autoSize();
                Log.i(TAG, "onTouchEvent: mState === "+mState);
                break;
        }
    ...
    public void autoSize() {
        ...
        if (maxHeaderViewHeight < currentHeight) {
            targetHeight += currentHeight - maxHeaderViewHeight;
        }
        if(mState==STATE_REFRESHING){
    ...
    private void setStateByHeight(int height,boolean isAuto) {
        ...
        if (height - headerViewHeight <2* headerViewHeight) {
            onStatusChange(STATE_NORMAL);
        } else if (height - headerViewHeight > 2*headerViewHeight) {
            onStatusChange(STATE_READY);
        } else if (height - headerViewHeight == 2*headerViewHeight&&!isOnTouching&&!isAuto) {
            onStatusChange(STATE_REFRESHING);
        }
    }
